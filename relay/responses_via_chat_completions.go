...
// responsesViaChatCompletions orchestrates a /v1/responses (or forced-from-responses)
// client request through an upstream /v1/chat/completions endpoint, then
// converts the upstream chat response back into Responses-API shape for the
// client.
func responsesViaChatCompletions(c *gin.Context, info *relaycommon.RelayInfo, adaptor channel.Adaptor, request *dto.OpenAIResponsesRequest) (*dto.Usage, *types.NewAPIError) {
	if request == nil {
		return nil, types.NewError(nil, types.ErrorCodeInvalidRequest, types.ErrOptionWithSkipRetry())
	}
